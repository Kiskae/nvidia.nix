...
def format_output(src_path: PurePosixPath, target_path: PurePosixPath,
                  permissions: str, file_type: str, module: Optional[str],
                  arch: Optional[str], extra: Optional[str]) -> str:
...
def resolve_symlink(src_path: PurePosixPath,
                    data: List[str]) -> Tuple[PurePosixPath, PurePosixPath]:
...
def maybe_pop(list: List[str], test: Callable[[str],
...
def pop_these_dirs(path: PurePosixPath,
                   dirs: List[str]) -> Tuple[PurePosixPath, Optional[str]]:
    segments = path.parts
...
def convert_manifest_entry(raw_line: str, legacy_kernel_depth: int = 1) -> str:
    ...
    return format_output(src_file, target_file, perms, type, module, arch,
                         extra)
...
def convert_manifest(source: Iterable[str], sink: StringIO):
    ...
    for line in it:
        try:
            print(convert_manifest_entry(line,
                                         legacy_kernel_depth=kernel_dir_depth),
                  file=sink)
        except IndexError:
